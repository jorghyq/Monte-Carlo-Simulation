...
int main()
{
	...
	start = clock();
	disp_array(1);
	//print_array(elements,num_total);
	// Begin to simulatte
			...
			if(ind_ele < num_molecule)
			{
				...
				while (state == 1)
				{
					...
					if ((is_occupied(&points_new[0],5)) == 0 && (is_forbidden(&points_new[0],4)) == 0)
					{
						...
						{
							//print_array(points_t2,5,"points_t2");
			...
			//if (ind_ele > num_molecule-1)
			{
				...
				while(state == 1)
				{
					...
					if (is_occupied(&points_new[0],1) == 0 && is_forbidden(&points_new[0],1) == 0)
					{
						...
						if(p > (double)rand()/RAND_MAX)
						{
							//*points_t1[0] = elements[ind_ele][0];
							//*(points_t1[0]+1) = elements[ind_ele][1];
							set_element(&points_old[4],1,0,ind_ele);
							//*points_t2[0] = new_pos[0];
							//*(points_t2[0]+1) = new_pos[1];
