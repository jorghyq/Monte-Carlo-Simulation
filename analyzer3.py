# analyzer for the BDS285
# for the Ev/Ec and Ev/kT
...
class McAnalyzer:
	...
	def load_txt(self,txt_name):
		...
		# DIFFERENCE THAN OTHER ANALYZER!!!!!
		self.cenergy = float(namedata[4])
		self.venergy = float(namedata[5])
		self.cenergy_ind = self.cenergy/self.venergy - 1
		self.venergy_ind = (self.venergy - self.venergy_init)/self.venergy_step
		sprint("self.cenergy_ind",self.cenergy_ind )
		sprint("self.venergy_ind",self.venergy_ind)
...
if __name__ == "__main__":
	...
	ind_nmetal = (nmetal_final - nmetal_initial)/nmetal_step + 1
	ind_cenergy = 10
	ind_venergy = (venergy_final - venergy_initial)/venergy_step + 1
	
	sprint("ind_venergy",ind_venergy)
	...
	#######################################
	# dmol_num: ind_venergy * ind_cenergy
	#   1) dense
	#   2) 1d
	#   3) 2d
	#	4) disordered
	...
	mdense = np.zeros((ind_venergy,ind_cenergy))
	m1d = np.zeros((ind_venergy,ind_cenergy))
	m2d = np.zeros((ind_venergy,ind_cenergy))
	mdis = np.zeros((ind_venergy,ind_cenergy))
	#totalenergy = np.zeros((ind_venergy,ind_cenergy))
	#cbond_num = np.zeros((ind_venergy,ind_cenergy))
	#vbond_num = np.zeros((ind_venergy,ind_cenergy))
	#cbond_num_av = np.zeros((ind_venergy,ind_cenergy))
	#totalenergy_av = np.zeros((ind_venergy,ind_cenergy))
	#cbond_num_avt = np.zeros((ind_venergy,ind_cenergy))
	#for line in files:
	...
	sprint("mdense.shape",mdense.shape)
	sprint("m1d.shape",m1d.shape)
	sprint("m2d.shape",m2d.shape)
	sprint("mdis.shape",mdis.shape)
	for line in files:
		if line[0] == '1' and line[-4:] == '.txt':
			analyzer.load_txt(line)
			mols0, count0 = analyzer.clustering(0)
			mdense[analyzer.venergy_ind][analyzer.cenergy_ind] = count0
			mols1, count1 = analyzer.clustering(1)
			m1d[analyzer.venergy_ind][analyzer.cenergy_ind] = count1
			mols2, count2 = analyzer.clustering(2)
			m2d[analyzer.venergy_ind][analyzer.cenergy_ind] = count2
			newlist = []
			count3 = 0
			mol_list = range(200)
			...
			for i in range(200):
				if i not in newlist:
					count3 = count3 + 1
			mdis[analyzer.venergy_ind][analyzer.cenergy_ind] = count3
	...
	#cbond_num = np.loadtxt("cbond_num.txt",delimiter=',')
	#cbond_num_av = np.loadtxt("cbond_num_av.txt",delimiter=',')
	#cbond_num_avt = np.loadtxt("cbond_num_avt.txt",delimiter=',')
	#vbond_num = np.loadtxt("vbond_num.txt",delimiter=',')
	#np.savetxt("totalenergy.txt",totalenergy,delimiter=',')	
	#np.savetxt("cbond_num.txt",cbond_num,delimiter=',')
	#np.savetxt("vbond_num.txt",vbond_num,delimiter=',')
	#np.savetxt("cbond_num_av.txt",cbond_num_av,delimiter=',')
	...
	m2d_p = m2d/mtotal
	mdis_p = mdis/mtotal
	xvenergy = np.array(range(venergy_initial,venergy_final+1,venergy_step))
	sprint("xvenergy.shape",xvenergy.shape)
	fig = plt.figure()
	fig.add_subplot(2,2,1)
	for i in range(0,mdense.shape[1]):
		plt.plot(xvenergy,mdense_p[:,i],label = 'cenerg = %d' % (i*2+1))
		plt.text(xvenergy[int(ind_venergy/2)],mdense_p[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	...
	for i in range(0,m1d.shape[1]):
		plt.plot(xvenergy,m1d_p[:,i],label = 'cenerg = %d' % (i*2+1))
		plt.text(xvenergy[int(ind_venergy/2)],m1d_p[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	plt.legend(loc=0,fontsize = 8)
	plt.xlabel('number of metals')
	...
	for i in range(0,m2d.shape[1]):
		plt.plot(xvenergy,m2d_p[:,i],label = 'cenerg = %d' % (i*2+1))
		plt.text(xvenergy[int(ind_venergy/2)],m2d_p[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	plt.legend(loc=0,fontsize = 8)
	plt.xlabel('number of metals')
	...
	for i in range(0,mdis.shape[1]):
		plt.plot(xvenergy,mdis_p[:,i],label = 'cenerg = %d' % (i*2+1))
		plt.text(xvenergy[int(ind_venergy/2)],mdis_p[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	plt.legend(loc=0,fontsize = 8)
	plt.xlabel('number of metals')
	...
	#for i in range(0,totalenergy.shape[1]):
		#plt.plot(xvenergy,totalenergy[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],totalenergy[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
	...
	#for i in range(0,cbond_num.shape[1]):
		#plt.plot(xvenergy,cbond_num[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],cbond_num[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
	...
	#for i in range(0,vbond_num.shape[1]):
		#plt.plot(xvenergy,vbond_num[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],vbond_num[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
	...
	#for i in range(0,cbond_num_av.shape[1]):
		#plt.plot(xvenergy,cbond_num_av[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],cbond_num_av[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
	...
	#for i in range(0,totalenergy_av.shape[1]):
		#plt.plot(xvenergy,totalenergy_av[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],totalenergy_av[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
	...
	#for i in range(0,cbond_num_avt.shape[1]):
		#plt.plot(xvenergy,cbond_num_avt[:,i],label = 'cenerg = %d' % (i*2+1))
		#plt.text(xvenergy[int(ind_venergy/2)],cbond_num_avt[int(ind_venergy/2),i], str(i*2+1),fontsize=8)
	#plt.legend(loc=0,fontsize = 8)
	#plt.xlabel('number of metals')
